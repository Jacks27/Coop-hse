...
class UserModel(BaseModel):
    ...
    passporturlstring = Strpatt(pat=r"^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/|www\.)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$")
    password = SizedString(minlen=8)
    tbl_colomns = ["firstname", "lastname", "othername", "email", "phonenumber"\
